...
** Last update Sun Jun  5 22:05:39 2016 Nicolas Zimmermann
*/
...
DIR	**init_dir(char **pathes, bool is_file)
{
  DIR	**dir;
  int	i;
  int	j;
  ...
  if (pathes)
    while (pathes[i++]);
  if (!(dir = malloc(sizeof(DIR *) * (i + is_file + 2))))
    return (NULL);
  i = 0;
  j = -1;
  if (pathes)
...
t_err		file_list(t_autocomp *autoc, char **pathes)
{
  ...
  if (!(autoc->head = malloc(sizeof(t_file))))
    return (print_error(ERROR_MALLOC_FAILED));
  if (!(dir = init_dir(pathes, autoc->is_file)))
    return (print_error(ERROR_MALLOC_FAILED));
